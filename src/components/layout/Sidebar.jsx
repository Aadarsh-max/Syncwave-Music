...
import { HiHome, HiSearch, HiCollection, HiPlus, HiUserGroup, HiSparkles, HiUser } from 'react-icons/hi'
...
const rooms = [
  { to: '/room/new', icon: HiUserGroup, label: 'Create Room' },
  { to: '/grow', icon: HiSparkles, label: 'Grow (AI)' },
]
  ...
  return (
    ...
    >
        ...
        {nav.map(({ to, icon: Icon, label, exact }) => (
          <NavLink
            ...
            style={({ isActive }) => ({
              ...
              background: isActive
                ? 'linear-gradient(135deg, rgba(29,185,84,0.15), rgba(14,165,233,0.08))'
                : 'transparent',
      ...
      <div className="px-3 mt-3">
        <p
          className="text-[10px] font-black uppercase px-3 mb-2"
          style={{ color: '#ffffff30', letterSpacing: '2px' }}
        >
          Rooms
        </p>
        <div className="flex flex-col gap-0.5">
          {rooms.map(({ to, icon: Icon, label }) => (
            <NavLink
              key={to}
              to={to}
              className="no-underline"
      ...
      <div className="px-3 mt-1">
        <button
          className="flex items-center gap-3 w-full cursor-pointer transition-all duration-200"
          style={{
            padding: '10px 12px',
            borderRadius: '12px',
            fontSize: '14px',
